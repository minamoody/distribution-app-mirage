...
with nav_tab2:
    ...
    if 'processed_orders' in st.session_state:
        ...
        st.info("Run the AI Dispatcher in Tab 1 first.")

# TAB 3: WHATSAPP PORTAL
with nav_tab3:
...
# TAB 5: TECHNICIAN DATABASE & BULK UPLOAD
with nav_tab5:
    st.header("⚙️ Master Technician Database")
    
    st.subheader("📤 Bulk Upload Technicians Sheet")
    uploaded_tech_file = st.file_uploader("Upload Technician Database (.xlsx or .csv)", type=["xlsx", "csv"], key="tech_uploader")
    
    if uploaded_tech_file:
        try:
            new_tech_df = pd.read_csv(uploaded_tech_file) if uploaded_tech_file.name.endswith('.csv') else pd.read_excel(uploaded_tech_file)
            st.write("Preview of Uploaded Technicians:")
            st.dataframe(new_tech_df.head(), use_container_width=True)
            
            if st.button("📥 Import & Replace Current Database", type="primary"):
                new_tech_df.columns = [c.lower().strip() for c in new_tech_df.columns]
                save_tech_df(new_tech_df)
                st.success("✅ Technician Database updated successfully!")
                st.rerun()
        except Exception as e:
            st.error(f"Error reading file: {e}")
            
    st.markdown("---")
    st.subheader("✏️ Edit & Manage Technicians")
